...
after the previous one, like successive shots in a film, so the story
must visibly advance by that much between frames. Frame 1 is the first
beat AFTER the source image, not a copy of it.
...
- Split the story into N beats of roughly equal narrative weight, each
  ...
  the interval is long; skip the in-between motion like a film cut.
- Only the ACTION/situation clause changes between frames; subject
  identity, setting continuity and style stay fixed. Camera and
  composition may change between frames like distinct shots, as long as
  the world stays recognizably the same.
