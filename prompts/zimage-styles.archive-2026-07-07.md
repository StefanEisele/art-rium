> ARCHIVED 2026-07-07 — superseded by the 4-style moodboard-derived library
> in `zimage-styles.md`. Kept for reference only; not read by any code
> (`_read_prompt("zimage-styles.md")` only ever loads the live file).
...
## Style H — Autopoietic Paint Dissolve (Blumquist Infusion)
*Classical anatomical figures born from a swirling rust-and-blue paint
vortex, dissolved by iridescent film and long-exposure ghosting.*

Studio three-quarter or full-figure view on a 50mm f/1.8, medium-format
Hasselblad rendering, on-camera flash dragged with a long exposure so a
faint ghosted double of the body trails the movement, atmospheric haze
in the air. A single Baroque-anatomy figure — marble-pale flesh,
sculptural fabric drapery — emerges mid-gesture from a viscous vortex of
liquid paint, its crown-splash rims and coiling ribbons freezing in the
air as they coalesce into the body. A thin iridescent oil-slick sheen
glazes the surface with subtle rainbow light, and the face is partly
dissolved into motion blur, identity unresolved. Palette of rust orange
and deep dim blue, desaturated to a filmic, analog grade against a bare
sage-grey studio wall falling into deep shadow, dusty highlights,
bone-white drapery. Texture: glossy paint viscosity, mid-air droplets,
fine marbling, satin sheen, hyperreal pore and skin detail against
liquid abstraction, Cinestill film grain and gentle halation. Mood:
classical, kinetic, uncanny, melancholic, timeless. Anchors: Alberto
Seveso ink figuration, Paolo Roversi long-exposure editorial.
...
- Cycle A → B → C → D → E → F deterministically across the variants
  ...
  surface evenly across batches. Use H as the default seventh style —
  an uncanny blumquist-infused sibling of C and F, same rust/blue
  palette with an iridescent, identity-dissolving finish. Use G only
  when the subject is a landscape or environment with no human figure.
